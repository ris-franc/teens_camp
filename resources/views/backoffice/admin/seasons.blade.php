...
<div class="container-fluid">
    <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between mb-4 gap-3">
        ...
        <button type="button" class="btn btn-camp-red btn-sm w-100 w-sm-auto py-2 px-3 fw-semibold text-nowrap" data-bs-toggle="modal" data-bs-target="#createSeasonModal" style="min-height: 44px;">
            <i class="bi bi-calendar-plus me-1"></i> Create New Camp Season
        </button>
    </div>

    <!-- Seasons List Table (Desktop) & Cards (Mobile) -->
            ...
            @foreach($seasons as $s)
                <div class="camp-card p-3 border border-secondary border-opacity-25 shadow-sm">
                    <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
                        <div>
                            <div class="fw-bold text-danger fs-6">{{ $s->name }}</div>
                            <div class="small text-muted">{{ $s->venue }}</div>
                        </div>
                        <div class="text-end">
                            <span class="badge bg-dark border text-white mb-1">{{ $s->year }}</span>
                            <div>
                                @if($s->status === 'active')
                    ...
                    @if($s->theme)
                        <div class="p-2 rounded bg-black bg-opacity-25 border border-secondary border-opacity-25 small text-white-50 mb-2" style="font-size: 11px;">
                            <i class="bi bi-quote text-danger me-1"></i>{{ $s->theme }}
                        </div>
                    @endif
                    ...
                    <div class="row g-2 py-2 my-2 border-top border-bottom border-secondary border-opacity-25 bg-black bg-opacity-25 rounded px-2 small">
                        <div class="col-6">
                            ...
                            <span class="fw-bold text-white fs-6">KES {{ number_format($s->price, 0) }}</span>
                        </div>
                        <div class="col-6 text-end">
                            <span class="text-muted d-block" style="font-size: 10px;">REGISTRATIONS</span>
                            <span class="fw-bold text-danger fs-6">{{ $s->registrations_count }} <span class="text-muted fw-normal" style="font-size: 11px;">/ {{ $s->capacity }}</span></span>
                        </div>
                        <div class="col-12 mt-1">
                            <span class="text-muted d-block" style="font-size: 10px;">DATES</span>
                            <span class="text-white-50"><i class="bi bi-calendar3 text-danger me-1"></i>{{ $s->start_date->format('M d') }} - {{ $s->end_date->format('M d, Y') }}</span>
                        </div>
                    </div>

                    @php
                        $capPercent = $s->capacity > 0 ? min(100, round(($s->registrations_count / $s->capacity) * 100)) : 0;
                    @endphp
                    <div class="mb-3">
                        <div class="d-flex justify-content-between small text-muted mb-1" style="font-size: 11px;">
                            <span>Capacity Fill: {{ $capPercent }}%</span>
                            <span>{{ $s->registrations_count }} of {{ $s->capacity }} campers</span>
                        </div>
                        <div class="camp-progress" style="height: 6px;">
                            <div class="camp-progress-bar" style="width: {{ $capPercent }}%;"></div>
                        </div>
                    </div>

                    <div class="d-flex gap-2 flex-wrap pt-2 border-top border-secondary border-opacity-25">
                        <button type="button" class="btn btn-outline-secondary btn-sm flex-fill py-2" data-bs-toggle="modal" data-bs-target="#editSeasonModal-{{ $s->id }}" style="min-height: 42px;">
                            <i class="bi bi-pencil-square me-1"></i> Edit
                        </button>
                        <button type="button" class="btn btn-outline-danger btn-sm flex-fill py-2" data-bs-toggle="modal" data-bs-target="#brandingModal-{{ $s->id }}" style="min-height: 42px;">
                            <i class="bi bi-image me-1"></i> Poster &amp; Values
                        </button>
                        <a href="{{ route('backoffice.switch-season', $s) }}" class="btn btn-camp-dark btn-sm flex-fill py-2 text-center d-inline-flex align-items-center justify-content-center text-decoration-none" style="min-height: 42px;">
                            <i class="bi bi-eye me-1"></i> View Data
                        </a>
                    </div>
                </div>
    ...
    <!-- Modals for Seasons (Accessible by both Desktop Table and Mobile Cards) -->
    @foreach($seasons as $s)
        <!-- Edit Season Modal -->
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content border-0" style="background:#1a1a1e; color:#fff;">
                <form action="{{ route('backoffice.admin.seasons.create') }}" method="POST">
                    @csrf
                    <div class="modal-header border-bottom border-secondary border-opacity-25">
                        <h5 class="modal-title fw-bold text-white"><i class="bi bi-calendar-plus text-danger me-2"></i> Create New Camp Season</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-semibold small">Theme / Motto</label>
